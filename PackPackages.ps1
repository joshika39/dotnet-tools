...
if ( -not (Test-Path $WorkDir\ManagerCore.ps1)) {
    Write-Output "Error: Missing Core scripts file."
    exit 1
}

$nugetExePath = "$WorkDir\nuget.exe"
if ( -not (Test-Path $nugetExePath)) {
    Write-Output "Error: Missing nuget.exe file."
    exit 1
}
...
if(-not $DebugPreference -and -not (Test-Path $env:NUSPEC_DIR\tmp)){
    Write-Output "Info: Created temp folder: $env:NUSPEC_DIR\tmp"
...
foreach ($projectData in $data) {
    ...
    if (-not $DebugPreference) {
        ...
        Write-Output "Info: Copied: $env:NUSPEC_DIR\Projects\$project.nuspec -> $env:NUSPEC_DIR\tmp\$project.nuspec"
        ...
        Write-Output "Info: $project.nuspec set up with $env:BUILD_CONFIG build configuration"
        ...
        Write-Output "(Debug) Info: Created temp folder: $env:NUSPEC_DIR\tmp"
        Write-Output "(Debug) Info: Copied: $env:NUSPEC_DIR\Projects\$project.nuspec -> $env:NUSPEC_DIR\tmp\$project.nuspec"
        Write-Output "(Debug) Info: $project.nuspec set up with $env:BUILD_CONFIG build configuration"
        Write-Output "(Debug) Info: Packing $env:NUSPEC_DIR\tmp\$project.nuspec with version $version to $env:NUSPEC_DIR\Packages\$project\$versionDir\"
        Write-Output ""
    }
}
